...
const sectionContent = (
  ...
  jobs: <div><h5>Jobs</h5><hr /><p>Browse job openings for students and freshers.</p></div>,
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      console.log('Token:', token); // Add this line temporarily to debug
      if (!token) return;
      try {
        ...
        if (res.ok) {
          ...
          setUser({
            ...
            branch: data.branch,
            year: data.year,
            image: data.image || null, // if you store image in backend
          });
        }
      } catch (err) {
        // handle error
  ...
  const handleSaveProfile = async (e) => {
    ...
    // Only update if user.image is set (i.e., a new image was chosen)
    if (user.image) {
      const token = localStorage.getItem('token');
      try {
        ...
        if (res.ok) {
          // Optionally update user state with returned user data
  ...
  return (
    <div className="min-vh-100" style={{ background: "linear-gradient(120deg, #f4f6fb 60%, #e3e9f7 100%)" }}>
      ...
      <nav className="navbar navbar-expand-lg navbar-light bg-white shadow-sm px-4">
        <span className="navbar-brand fw-bold fs-4">Student Dashboard</span>
